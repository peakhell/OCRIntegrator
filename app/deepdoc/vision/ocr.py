...
from app.utils.singal import SingletonMeta
...
def create_operators(op_param_list, global_config=None):
    ...
    Args:
        :param op_param_list: a dict list, used to create some operators
        :param global_config:
    """
...
def load_model(model_dir, nm):
    ...
    if ort.get_device() == "GPU":
        logger.info(f"load {nm} model using GPU")
...
class TextRecognizer(metaclass=SingletonMeta):
    _predictor = None
    _input_tensor = None
    ...
    def __init__(self, model_dir):
        ...
        if TextRecognizer._input_tensor is None or TextRecognizer._predictor is None:
            if torch.cuda.is_available():
                TextRecognizer._predictor, TextRecognizer._input_tensor = load_model_cuda(model_dir, 'rec')
            else:
                TextRecognizer._predictor, TextRecognizer._input_tensor = load_model(model_dir, 'rec')
    ...
    def resize_norm_img(self, img, max_wh_ratio):
        ...
        # w = TextRecognizer._input_tensor.shape[3:][0]
        # if isinstance(w, str):
        #     pass
    ...
    def __call__(self, img_list):
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            try:
                outputs = TextRecognizer._predictor.run(None, input_dict)
...
class TextDetector(metaclass=SingletonMeta):
    _predictor = None
    _input_tensor = None
    ...
    def __init__(self, model_dir):
        ...
        if TextDetector._predictor is None or TextDetector._input_tensor is None:
            if torch.cuda.is_available():
                TextDetector._predictor, TextDetector._input_tensor = load_model_cuda(model_dir, 'det')
            else:
                TextDetector._predictor, TextDetector._input_tensor = load_model(model_dir, 'det')
        self.postprocess_op = build_post_process(postprocess_params)

        # img_h, img_w = TextDetector._input_tensor.shape[2:]
...
class OCR(object):
    ...
    def recognize(self, ori_im, boxes):
        img_crops = []
        for box in boxes:
