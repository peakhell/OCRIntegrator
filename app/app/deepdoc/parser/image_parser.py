...
from app.deepdoc.vision import OCR, Recognizer, LayoutRecognizer, TableStructureRecognizer
from app.deepdoc.rag.nlp import rag_tokenizer
...
class ImageParser:
    ...
    def _naive_vertical_merge(self):
        ...
        while i + 1 < len(bxs):
            ...
            if (any(feats) and not any(concatting_feats)) or any(detach_feats):
                # logger.info( b["text"], b_["text"], any(feats), any(concatting_feats), any(detach_feats))
                i += 1
                continue
            # merge up and down
            b["bottom"] = b_["bottom"]
    ...
    def _line_tag(self, bx, ZM):
        ...
        while bott * ZM > self.page_images[pn[-1] - 1].shape[1]:
    ...
    def __filterout_scraps(self, boxes, ZM):
        ...
        while boxes:
            lines = []
            widths = []
            pw = self.page_images[boxes[0]["page_number"] - 1].shape[0] / ZM
